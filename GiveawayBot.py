...
async def giveaway_watcher():
    ...
    while not bot.is_closed():
        ...
        async with get_db() as db:
            async with db.execute(
                "SELECT message_id FROM giveaways WHERE ended = 0 AND end_time <= ?", (now,)
            ) as cursor:
                rows = await cursor.fetchall()
        ...
        async with get_db() as db:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS item_store (
                    item_name TEXT PRIMARY KEY, price INTEGER, role_id INTEGER, description TEXT
                )
            """)
            await db.execute("""
...
# Must be defined before on_message which references it.
...
async def giveawayroles(interaction: discord.Interaction):
    async with get_db() as db:
        async with db.execute(
            "SELECT role_id FROM giveaway_roles WHERE guild_id = ?", (interaction.guild.id,)
        ) as cursor:
            rows = await cursor.fetchall()
    if not rows:
        await interaction.response.send_message("❌ No giveaway roles configured.")
        return
    mentions = [interaction.guild.get_role(row[0]).mention for row in rows if interaction.guild.get_role(row[0])]
...
async def gift(interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    async with db_lock:
        async with get_db() as db:
            async with db.execute("SELECT balance FROM balances WHERE user_id = ?", (interaction.user.id,)) as cursor:
                data = await cursor.fetchone()
            if data is None or data[0] < amount:
                await interaction.response.send_message("❌ You don't have enough balance.", ephemeral=True); return
            await db.execute("UPDATE balances SET balance = balance - ? WHERE user_id = ?", (amount, interaction.user.id))
            ...
            await db.execute("UPDATE user_stats SET gifted_balance = gifted_balance + ? WHERE user_id = ?", (amount, interaction.user.id))
            await db.commit()
...
async def get_exp(user_id):
    ...
    async with get_db() as db:
        async with db.execute(
            "SELECT SUM(amount) FROM exp_history WHERE user_id = ? AND timestamp >= ?", (user_id, week_ago)
        ) as cursor:
            data = await cursor.fetchone()
...
async def get_level(user_id):
    ...
    return min((exp // LEVEL_DIVISOR) + 1, 100)

async def get_spent_exp(user_id):
    async with get_db() as db:
        async with db.execute("SELECT amount FROM spent_exp WHERE user_id = ?", (user_id,)) as cursor:
            data = await cursor.fetchone()
        if not data:
...
async def add_spent_exp(user_id, amount):
    ...
    async with db_lock:
        async with get_db() as db:
            await db.execute("UPDATE spent_exp SET amount = ? WHERE user_id = ?", (current + amount, user_id))
            await db.commit()

# ---------------- MESSAGE EXP + GAME GUESS CHECK ---------------- #
...
async def on_message(message):
    ...
    if now - last_time >= 30:
        ...
        gained = min(50, exp_gain + random.randint(0, max(1, bonus)))
...
@app_commands.describe(
    prize="Prize name", minutes="Duration in minutes", winners="Number of winners",
    reward="Balance reward", channel="Channel to post in (defaults to current)",
    required_role="Required role to enter", template="Template color"
)
@command_enabled()
async def giveaway(
    interaction: discord.Interaction, prize: str, minutes: int, winners: int,
    reward: int, channel: discord.TextChannel = None,
    ...
    async with get_db() as db:
        await db.execute(
            ...
            (message.id, target_channel.id, prize, winners, reward, int(end_time.timestamp()),
             required_role.id if required_role else 0, template, 0)
        )
        await db.commit()
...
async def end_giveaway(message_id, reroll=False):
    async with db_lock:
        async with get_db() as db:
            ...
            (message_id, channel_id, prize, winner_count, reward, end_time, required_role, template, ended) = data
    ...
    async with db_lock:
        async with get_db() as db:
            ...
            for winner in winners:
                ...
                await db.execute("UPDATE balances SET balance = balance + ? WHERE user_id = ?", (reward, winner.id))
                await db.execute("INSERT OR REPLACE INTO giveaway_winners VALUES (?, ?, ?)", (message_id, winner.id, reward))
...
async def reroll(interaction: discord.Interaction, message_id: str):
    ...
    async with db_lock:
        async with get_db() as db:
            ...
            (_mid, channel_id, prize, winner_count, reward, end_time, required_role, template, ended) = data
            async with db.execute("SELECT winner_id, reward FROM giveaway_winners WHERE message_id = ?", (message_id,)) as cursor:
                old_data = await cursor.fetchone()
    ...
    if not reaction:
        await interaction.response.send_message("❌ Reaction not found."); return
    ...
    async with db_lock:
        async with get_db() as db:
            if old_data:
                ...
                await db.execute("INSERT OR IGNORE INTO balances VALUES (?, 0)", (old_winner_id,))
                await db.execute("UPDATE balances SET balance = MAX(0, balance - ?) WHERE user_id = ?", (old_reward, old_winner_id))
            await db.execute("INSERT OR IGNORE INTO balances VALUES (?, 0)", (new_winner.id,))
            await db.execute("UPDATE balances SET balance = balance + ? WHERE user_id = ?", (reward, new_winner.id))
            await db.execute("INSERT OR REPLACE INTO giveaway_winners VALUES (?, ?, ?)", (message_id, new_winner.id, reward))
            await db.commit()
...
async def get_tickets(guild_id, user_id):
    async with db_lock:
        async with get_db() as db:
            async with db.execute("SELECT tickets FROM raffle WHERE guild_id = ? AND user_id = ?", (guild_id, user_id)) as cursor:
                data = await cursor.fetchone()
            if not data:
...
async def add_tickets(guild_id, user_id, amount):
    ...
    async with db_lock:
        async with get_db() as db:
            await db.execute("UPDATE raffle SET tickets = ? WHERE guild_id = ? AND user_id = ?", (new_tickets, guild_id, user_id))
            await db.commit()
...
async def buytickets(interaction: discord.Interaction, amount: int):
    if amount <= 0:
        await interaction.response.send_message("❌ Amount must be greater than 0."); return
...
async def setrafflechannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    async with db_lock:
        async with get_db() as db:
            await db.execute("INSERT OR REPLACE INTO raffle_config VALUES (?, ?)", (interaction.guild.id, channel.id))
            await db.commit()
    await interaction.response.send_message(f"✅ Raffle announcements will now be sent to {channel.mention}")
...
async def raffle_loop():
    ...
    while not bot.is_closed():
        ...
        for guild in bot.guilds:
            async with get_db() as db:
                async with db.execute("SELECT user_id, tickets FROM raffle WHERE guild_id = ?", (guild.id,)) as cursor:
            ...
            async with get_db() as db:
                async with db.execute("SELECT channel_id FROM raffle_config WHERE guild_id = ?", (guild.id,)) as cursor:
            ...
            if announce_channel:
                await announce_channel.send(f"🎉 <@{winner_id}> won the daily raffle and will receive a huge pet!")
...
async def addautogiveaway(interaction: discord.Interaction, prize: str, reward: int, winners: int):
    ...
    await interaction.response.send_message(f"✅ Added auto giveaway: {prize} | Reward: {reward} | Winners: {winners}")
...
async def startgiveaways(
    interaction: discord.Interaction, interval_seconds: int,
    ...
    if auto_giveaway_task and not auto_giveaway_task.done():
        await interaction.response.send_message("Automatic giveaways are already running.", ephemeral=True); return
    ...
    async def auto_loop():
        ...
        while AUTO_GIVEAWAY_ENABLED:
            ...
            prize, reward, winners = gd["prize"], gd["reward"], gd["winners"]
            ...
            async with get_db() as db:
                await db.execute(
                    "INSERT INTO giveaways (message_id, channel_id, prize, winners, reward, end_time, required_role, template, ended) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (message.id, target_channel.id, prize, winners, reward, int(end_time.timestamp()), 0, "gold", 0)
                )
                await db.commit()
...
async def chest(interaction: discord.Interaction, amount: int = 1):
    ...
    if amount <= 0:
        await interaction.followup.send("❌ Amount must be greater than 0."); return
    ...
    if exp < total_cost:
        await interaction.followup.send(f"❌ You need {total_cost:,} EXP."); return
    ...
    for _ in range(amount):
        prize = random.choices(CHEST_PRIZES, weights=[p["chance"] for p in CHEST_PRIZES], k=1)[0]
        name = prize["name"]
    ...
    async with db_lock:
        async with get_db() as db:
            await db.execute("INSERT OR IGNORE INTO spent_exp VALUES (?, 0)", (interaction.user.id,))
            await db.execute("UPDATE spent_exp SET amount = amount + ? WHERE user_id = ?", (total_cost, interaction.user.id))
            if total_balance > 0:
            ...
            await db.execute("INSERT OR IGNORE INTO user_stats (user_id) VALUES (?)", (interaction.user.id,))
            await db.execute("UPDATE user_stats SET chests_opened = chests_opened + ? WHERE user_id = ?", (amount, interaction.user.id))
            await db.commit()
...
async def add_item(item_name, price, role_id, description):
    async with db_lock:
        async with get_db() as db:
            await db.execute("INSERT OR REPLACE INTO item_store VALUES (?, ?, ?, ?)", (item_name, price, role_id, description))
            await db.commit()
...
async def get_item(item_name):
    async with get_db() as db:
        async with db.execute("SELECT * FROM item_store WHERE LOWER(item_name) = LOWER(?)", (item_name,)) as cursor:
...
async def item_give(interaction: discord.Interaction, user: discord.Member, name: str, quantity: int = 1):
    ...
    if not item:
        await interaction.response.send_message("❌ Item not found in the store."); return
    await inventory_add(user.id, item[0], quantity)
    await interaction.response.send_message(f"✅ Gave **{quantity}x {item[0]}** to {user.mention}.")
...
async def item_take(interaction: discord.Interaction, user: discord.Member, name: str, quantity: int = 1):
    ...
    if not item:
        await interaction.response.send_message("❌ Item not found in the store."); return
    removed = await inventory_remove(user.id, item[0], quantity)
    if not removed:
        await interaction.response.send_message(f"❌ {user.mention} doesn't have {quantity}x **{item[0]}**."); return
    await interaction.response.send_message(f"🗑 Took **{quantity}x {item[0]}** from {user.mention}.")
...
async def disablecmd(interaction: discord.Interaction, command_name: str):
    ...
    if command_name in ("disablecmd", "enablecmd"):
        await interaction.response.send_message("❌ You cannot disable the enable/disable commands.", ephemeral=True); return
    disabled_commands.add(command_name)
    await interaction.response.send_message(f"🔒 Command `/{command_name}` has been disabled.")
...
async def enablecmd(interaction: discord.Interaction, command_name: str):
    ...
    if command_name not in disabled_commands:
        await interaction.response.send_message(f"ℹ️ `/{command_name}` is not currently disabled.", ephemeral=True); return
    disabled_commands.discard(command_name)
    await interaction.response.send_message(f"🔓 Command `/{command_name}` has been re-enabled.")
...
async def setraredropchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(f"✅ Rare drop announcements will be posted in {channel.mention}.\nWatching for: {prizes_listed}")
...
@app_commands.describe(role="Role to boost", boost="Boost % — e.g. 1.5 = +1.5%, -25 = -25% (penalty). All matching roles are summed.")
@command_enabled()
...
class TradeOffer:
    ...
    def display(self) -> str:
        ...
        if self.tickets > 0: lines.append(f"🎟 {self.tickets:,} raffle ticket(s)")
...
class TradeSession:
    ...
    def build_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        embed.add_field(name=f"{init_name}'s offer {init_status}", value=init_offer.display() if init_offer else "*Not set yet*", inline=True)
        embed.add_field(name=f"{tgt_name}'s offer {tgt_status}", value=tgt_offer.display() if tgt_offer else "*Not set yet*", inline=True)
...
class TradeOfferModal(discord.ui.Modal, title="Set Your Trade Offer"):
    balance_input = discord.ui.TextInput(label="Balance to offer (0 for none)", placeholder="e.g. 500", required=True, default="0", max_length=20)
    exp_input = discord.ui.TextInput(label="EXP to offer (0 for none)", placeholder="e.g. 1000", required=True, default="0", max_length=20)
    tickets_input = discord.ui.TextInput(label="Raffle tickets to offer (0 for none)", placeholder="e.g. 5", required=True, default="0", max_length=20)
    items_input = discord.ui.TextInput(label="Items/boxes to offer (blank for none)", placeholder="ItemName:qty, BoxName:qty", required=False, max_length=300)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("❌ Invalid balance amount.", ephemeral=True); return
        try:
            exp = int(self.exp_input.value.strip())
            if exp < 0: raise ValueError
        except ValueError:
            await interaction.response.send_message("❌ Invalid EXP amount.", ephemeral=True); return
        try:
            tickets = int(self.tickets_input.value.strip())
            if tickets < 0: raise ValueError
        except ValueError:
            await interaction.response.send_message("❌ Invalid ticket amount.", ephemeral=True); return
        ...
        if raw:
            for part in raw.split(","):
                ...
                try:
                    ...
                    await interaction.response.send_message(f"❌ Invalid qty for `{item_name}`.", ephemeral=True); return
                items.append((item_name, qty))

        if balance > 0:
            user_bal = await get_balance(user_id)
            if user_bal < balance:
                await interaction.response.send_message(f"❌ You only have {user_bal:,} coins.", ephemeral=True); return
        if exp > 0:
            user_exp = await get_exp(user_id)
            if user_exp < exp:
                await interaction.response.send_message(f"❌ You only have {user_exp:,} usable EXP.", ephemeral=True); return
        if tickets > 0:
            user_tickets = await get_tickets(session.guild_id, user_id)
            if user_tickets < tickets:
                await interaction.response.send_message(f"❌ You only have {user_tickets:,} raffle tickets.", ephemeral=True); return
        if items:
            user_inv = {n.lower(): q for n, q in await inventory_get(user_id)}
            for item_name, qty in items:
                if user_inv.get(item_name.lower(), 0) < qty:
                    await interaction.response.send_message(f"❌ You only have {user_inv.get(item_name.lower(), 0)}x **{item_name}**.", ephemeral=True); return
...
class TradeView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id not in (self.session.initiator_id, self.session.target_id):
            await interaction.response.send_message("❌ You're not part of this trade.", ephemeral=True); return False
        if self.session.done:
            await interaction.response.send_message("❌ This trade is already finished.", ephemeral=True); return False
        return True
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if session.offers[user_id] is None:
            await interaction.response.send_message("❌ Set your offer before confirming.", ephemeral=True); return
        async with session.lock:
            session.confirmed[user_id] = True
            if not all(session.confirmed.values()):
                embed = session.build_embed(interaction.guild)
                await session.message.edit(embed=embed, view=self)
                await interaction.response.send_message("✅ You confirmed. Waiting for the other party.", ephemeral=True); return
            # Both confirmed — execute
            success, error_msg = await execute_trade(session)
            session.done = True
            trade_sessions.pop(session.session_key(), None)
            if success:
                embed = discord.Embed(title="✅ Trade Complete!", description="The trade has been executed successfully.", color=discord.Color.green())
            else:
                embed = discord.Embed(title="❌ Trade Failed", description=error_msg, color=discord.Color.red())
            await session.message.edit(embed=embed, view=None)
            await interaction.response.send_message("Trade processed!" if success else f"Trade failed: {error_msg}", ephemeral=True)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        session.done = True
        trade_sessions.pop(session.session_key(), None)
        embed = discord.Embed(title="❌ Trade Cancelled", description=f"Trade cancelled by {interaction.user.mention}.", color=discord.Color.red())
        await session.message.edit(embed=embed, view=None)
    ...
    async def on_timeout(self):
        ...
        if not session.done:
            session.done = True
            trade_sessions.pop(session.session_key(), None)
            embed = discord.Embed(title="⏰ Trade Expired", description="This trade timed out after 5 minutes.", color=discord.Color.light_grey())
            if session.message:
                try:
                    await session.message.edit(embed=embed, view=None)
                except Exception:
...
async def execute_trade(session: TradeSession) -> tuple[bool, str]:
    ...
    for user_id, offer in [(init_id, init_offer), (tgt_id, tgt_offer)]:
        if offer.balance > 0 and await get_balance(user_id) < offer.balance:
            return False, f"<@{user_id}> no longer has enough balance."
        if offer.exp > 0 and await get_exp(user_id) < offer.exp:
            return False, f"<@{user_id}> no longer has enough EXP."
        if offer.tickets > 0 and await get_tickets(session.guild_id, user_id) < offer.tickets:
            return False, f"<@{user_id}> no longer has enough raffle tickets."
    ...
    if init_offer.balance > 0:
        await add_balance(init_id, -init_offer.balance); await add_balance(tgt_id, init_offer.balance)
    if tgt_offer.balance > 0:
        await add_balance(tgt_id, -tgt_offer.balance); await add_balance(init_id, tgt_offer.balance)
    if init_offer.exp > 0:
        await add_spent_exp(init_id, init_offer.exp); await add_exp(tgt_id, init_offer.exp)
    if tgt_offer.exp > 0:
        await add_spent_exp(tgt_id, tgt_offer.exp); await add_exp(init_id, tgt_offer.exp)
    if init_offer.tickets > 0:
        await add_tickets(session.guild_id, init_id, -init_offer.tickets)
        await add_tickets(session.guild_id, tgt_id, init_offer.tickets)
    if tgt_offer.tickets > 0:
        await add_tickets(session.guild_id, tgt_id, -tgt_offer.tickets)
        await add_tickets(session.guild_id, init_id, tgt_offer.tickets)
    for item_name, qty in init_offer.items:
        await inventory_remove(init_id, item_name, qty); await inventory_add(tgt_id, item_name, qty)
    for item_name, qty in tgt_offer.items:
        await inventory_remove(tgt_id, item_name, qty); await inventory_add(init_id, item_name, qty)
    return True, ""
...
@app_commands.describe(
    box="Box name", prize_type="Type of prize",
    chance="Weight for this prize (e.g. 50 = 50 out of total weight)",
    amount="Amount of balance or EXP", item_name="Item name (for 'item' type)",
    custom_label="Label for 'nothing' or 'custom' prizes"
)
...
async def addboxprize(
    interaction: discord.Interaction, box: str, prize_type: str, chance: int,
    amount: int = 0, item_name: str = None, custom_label: str = None
):
    if not await is_allowed_to_giveaway(interaction):
        await interaction.response.send_message("❌ No permission.", ephemeral=True); return
    if chance <= 0:
        await interaction.response.send_message("❌ Chance/weight must be > 0.", ephemeral=True); return
    async with get_db() as db:
    ...
    if prize_type in ("balance", "exp"):
        if amount <= 0:
            await interaction.response.send_message(f"❌ Provide a positive `amount` for {prize_type} prizes.", ephemeral=True); return
        prize_value = str(amount)
    elif prize_type == "item":
        if not item_name:
            await interaction.response.send_message("❌ Provide `item_name` for item prizes.", ephemeral=True); return
        item = await get_item(item_name)
        if not item:
            await interaction.response.send_message(f"❌ Item **{item_name}** not found in the store.", ephemeral=True); return
        prize_value = item[0]
    elif prize_type == "nothing":
        prize_value = custom_label or "Nothing"
    else:
        if not custom_label:
            await interaction.response.send_message("❌ Provide `custom_label` for custom prizes.", ephemeral=True); return
        prize_value = custom_label

    async with db_lock:
        async with get_db() as db:
            await db.execute(
                "INSERT INTO abuse_box_prizes (guild_id, box_name, prize_type, prize_value, prize_amount, chance) VALUES (?, ?, ?, ?, ?, ?)",
...
async def listboxes(interaction: discord.Interaction, box: str = None):
    async with get_db() as db:
        if box:
            ...
            async with db.execute("SELECT box_name FROM abuse_boxes WHERE guild_id = ?", (interaction.guild.id,)) as cursor:
                boxes = await cursor.fetchall()
    if not boxes:
...
async def givebox(interaction: discord.Interaction, box: str, role: discord.Role, amount: int = 1):
    ...
    if not members:
        await interaction.response.send_message(f"❌ No members found with {role.mention}.", ephemeral=True); return
    await interaction.response.defer()
...
async def openbox(interaction: discord.Interaction, box: str):
    ...
    if box.lower() not in owned or owned[box.lower()][1] < 1:
        await interaction.response.send_message(f"❌ You don't have **{box}** in your inventory.", ephemeral=True); return
    ...
    async with get_db() as db:
        async with db.execute("SELECT box_name FROM abuse_boxes WHERE guild_id = ? AND box_name = ?", (interaction.guild.id, canonical_box)) as cursor:
            if not await cursor.fetchone():
                await interaction.response.send_message(f"❌ Box **{canonical_box}** no longer exists on this server.", ephemeral=True); return
        async with db.execute("SELECT prize_type, prize_value, prize_amount, chance FROM abuse_box_prizes WHERE guild_id = ? AND box_name = ?", (interaction.guild.id, canonical_box)) as cursor:
            prizes = await cursor.fetchall()
    if not prizes:
        await interaction.response.send_message(f"❌ **{canonical_box}** has no prizes configured.", ephemeral=True); return
    removed = await inventory_remove(interaction.user.id, canonical_box, 1)
    if not removed:
        await interaction.response.send_message("❌ Failed to open box.", ephemeral=True); return
    ...
    if prize_type == "balance":
        ...
        result_text = f"✨ **{prize_value}**"
    embed = discord.Embed(title=f"📦 {canonical_box} Opened!", description=f"You got: {result_text}", color=discord.Color.orange())
    embed.set_thumbnail(url=interaction.user.display_avatar.url)
    await interaction.response.send_message(embed=embed)

# ---------------- RANDOM GAMES SYSTEM ---------------- #

@bot.tree.command(name="addgame", description="Add a random game (question) to the pool")
@app_commands.describe(
    name="The question shown to players (e.g. 'Guess the color of the sky')",
    reward_balance="Balance reward for the winner (default 0)",
    reward_exp="EXP reward for the winner (default 0)"
)
@command_enabled()
async def addgame(interaction: discord.Interaction, name: str, reward_balance: int = 0, reward_exp: int = 0):
    ...
    await interaction.response.send_message(
        f"✅ Added game **{name}** (reward: {reward_balance:,} coins + {reward_exp:,} EXP).\n"
...
async def removegame(interaction: discord.Interaction, name: str):
    ...
    async with db_lock:
        async with get_db() as db:
            ...
            await db.commit()
    await interaction.response.send_message(f"🗑 Removed game **{name}** and all its answers.")

@bot.tree.command(name="addgameanswer", description="Add a valid answer to a game")
@app_commands.describe(game_name="The game to add an answer to", answer="A valid answer players can type")
@command_enabled()
async def addgameanswer(interaction: discord.Interaction, game_name: str, answer: str):
    ...
    async with db_lock:
        async with get_db() as db:
            ...
            await db.commit()
    await interaction.response.send_message(f"✅ Added answer `{answer}` to game **{game_name}**.")
...
async def listgames(interaction: discord.Interaction, game_name: str = None):
    async with get_db() as db:
        if game_name:
            async with db.execute("SELECT game_name, reward_balance, reward_exp FROM games WHERE guild_id = ? AND game_name = ?", (interaction.guild.id, game_name)) as cursor:
                games = await cursor.fetchall()
        else:
            async with db.execute("SELECT game_name, reward_balance, reward_exp FROM games WHERE guild_id = ?", (interaction.guild.id,)) as cursor:
                games = await cursor.fetchall()
    if not games:
    ...
    for (gname, reward_bal, reward_exp) in games:
        ...
        reward_text = []
        if reward_bal > 0: reward_text.append(f"💰 {reward_bal:,}")
        if reward_exp > 0: reward_text.append(f"⭐ {reward_exp:,}")
        reward_str = " + ".join(reward_text) if reward_text else "No reward"
        if answers:
            answer_lines = "\n".join(f"  `#{aid}` {ans}" for aid, ans in answers)
            value = f"Reward: {reward_str}\nAnswers:\n{answer_lines}"
        else:
            value = f"Reward: {reward_str}\n*No answers yet*"
        embed.add_field(name=f"🎯 {gname}", value=value, inline=False)
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="setgamechannel", description="Set the channel for random games and configure timing")
@app_commands.describe(
    channel="Channel where games will be posted",
    answer_time="Seconds players have to answer before the game expires (default 30)",
    interval_seconds="Seconds between games when running (default 60)"
)
@command_enabled()
async def setgamechannel(
    ...
    if not await is_allowed_to_giveaway(interaction):
        await interaction.response.send_message("❌ No permission.", ephemeral=True); return
    if answer_time < 5:
        await interaction.response.send_message("❌ Answer time must be at least 5 seconds.", ephemeral=True); return
    if interval_seconds < 10:
        await interaction.response.send_message("❌ Interval must be at least 10 seconds.", ephemeral=True); return
    async with db_lock:
        async with get_db() as db:
            await db.execute(
                "INSERT OR REPLACE INTO game_config (guild_id, channel_id, answer_time, interval_seconds) VALUES (?, ?, ?, ?)",
                (interaction.guild.id, channel.id, answer_time, interval_seconds)
            )
            await db.commit()
    await interaction.response.send_message(
        f"✅ Game channel set to {channel.mention}.\n"
        f"Answer time: **{answer_time}s** | Interval: **{interval_seconds}s**"
    )

@bot.tree.command(name="startgames", description="Start automatic random games in the configured channel")
@command_enabled()
async def startgames(interaction: discord.Interaction):
    ...
    if not config:
        await interaction.response.send_message("❌ Use `/setgamechannel` first to configure the game channel.", ephemeral=True); return
    async with get_db() as db:
        async with db.execute("SELECT game_name FROM games WHERE guild_id = ? AND enabled = 1", (guild_id,)) as cursor:
            games = await cursor.fetchall()
        if not games:
            await interaction.response.send_message("❌ No enabled games. Use `/addgame` to add some.", ephemeral=True); return
...
async def guild_game_loop(guild_id: int):
    """Runs the game loop for a single guild."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        # Load config
        async with get_db() as db:
        ...
        # Pick a random enabled game with at least one answer
        async with get_db() as db:
        ...
        for gname, reward_bal, reward_exp in games:
            ...
            if answers:
                eligible.append((gname, reward_bal, reward_exp, answers))

        if not eligible:
            await asyncio.sleep(interval_seconds); continue
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"⏰ You have **{answer_time} seconds** to answer."
            ),
        ...
        if session.get("answered") and session.get("winner"):
            ...
            if reward_bal > 0:
                await add_balance(winner.id, reward_bal)
            if reward_exp > 0:
                await add_exp(winner.id, reward_exp)
        ...
        # Wait before next game (minus the answer time already spent)
        remaining = max(0, interval_seconds - answer_time)
        await asyncio.sleep(remaining)

async def game_loop():
    """Master game loop — restarts guild loops that died unexpectedly."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        for guild_id, task in list(game_tasks.items()):
            if task.done():
                ...
                try:
                    exc = task.exception()
                    if exc:
                        print(f"[GameLoop] Guild {guild_id} task died: {exc}")
                except Exception:
                    pass
                game_tasks[guild_id] = asyncio.create_task(guild_game_loop(guild_id))
...
async def leaderboard(interaction: discord.Interaction, category: app_commands.Choice[str]):
    ...
    async with get_db() as db:
        if value == "current_exp":
            ...
            async with db.execute("SELECT user_id, tickets FROM raffle WHERE guild_id = ? ORDER BY tickets DESC LIMIT 10", (interaction.guild.id,)) as cursor:
                leaderboard_data = await cursor.fetchall()
        elif value == "balance":
            async with db.execute("SELECT user_id, balance FROM balances ORDER BY balance DESC LIMIT 10") as cursor:
                leaderboard_data = await cursor.fetchall()
        else:
            async with db.execute(f"SELECT user_id, {value} FROM user_stats ORDER BY {value} DESC LIMIT 10") as cursor:
                leaderboard_data = await cursor.fetchall()
    ...
    if not leaderboard_data:
        await interaction.response.send_message("❌ No leaderboard data found."); return

    title_map = {
        "total_exp": "🏆 Total EXP Leaderboard", "current_exp": "⭐ Current EXP Leaderboard",
        "balance": "💰 Balance Leaderboard", "raffle_tickets_bought": "🎟 Lifetime Tickets Leaderboard",
        "current_tickets": "🎫 Current Raffle Tickets Leaderboard", "chests_opened": "📦 Chests Opened Leaderboard",
